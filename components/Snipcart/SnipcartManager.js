...
const SnipcartManager = memo(() => {
  ...
  useEffect(() => {
    if (snipcartClient) {
      ...
      //Snipcart confirmed-happening when purchase
      snipcartClient.events.on('cart.confirmed', (cartConfirmResponse) => {
        orderCompleted(cartConfirmResponse)
      })

      // shipping selected
      snipcartClient.events.on('shipping.selected', (shippingMethod) => {
        onCheckoutOption({ step: 4, checkoutOption: shippingMethod })
      })
    }
  }, [snipcartClient])
  ...
  const processInventoryCheck = (productInventory) => {
    ...
    if (
      !productInventory.id?.toUpperCase()?.endsWith('E4') &&
